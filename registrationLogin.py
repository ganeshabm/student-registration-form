...
class Window1:
    def __init__(self,root,db,conn):
        ...
        self.signuplabel = Label(self.frame0, text="SignUp Form",fg="Green",padx=65, pady=10,justify="center",font=('Bradley Hand ITC', 20))
        self.signuplabel.grid(row=1)

        self.gmaillabel = Label(self.frame, text="Enter Your Gmail:", padx=10, pady=10,font=('Bradley Hand ITC', 12))
        self.gmaillabel.grid(row=0, column=0, sticky="ew")
        self.namelabel = Label(self.frame, text="Enter Your name:", padx=10, pady=10,font=('Bradley Hand ITC', 12))
        self.namelabel.grid(row=1, column=0, sticky="ew")
        self.phonelabel = Label(self.frame, text="Enter Your phone:", padx=10, pady=10,font=('Bradley Hand ITC', 12))
        self.phonelabel.grid(row=2, column=0, sticky="ew")
        self.passwordlabel = Label(self.frame, text="Password:", padx=10, pady=10,font=('Bradley Hand ITC', 12))
        self.passwordlabel.grid(row=3, column=0, sticky="ew")
        self.reenterpasswordlabel = Label(self.frame, text="Re-Enter Password:", padx=10, pady=10,font=('Bradley Hand ITC', 12))
        self.reenterpasswordlabel.grid(row=4, column=0,sticky="ew")
        self.errorlabel =Label(self.frame1, text="", padx=0, pady=0, fg="red",font=('Bradley Hand ITC', 12))
        self.errorlabel.grid(row=5, column=0, sticky="ew")
        self.gmailentry = Entry(self.frame,font=('Bradley Hand ITC', 12))
        self.gmailentry.grid(row=0, column=1, sticky="ew")
        self.nameentry = Entry(self.frame,font=('Bradley Hand ITC', 12))
        self.nameentry.grid(row=1, column=1, sticky="ew")
        self.phoneentry = Entry(self.frame,font=('Bradley Hand ITC', 12))
        self.phoneentry.grid(row=2, column=1, sticky="ew")
        self.passwordentry =Entry(self.frame, show="*",font=('Bradley Hand ITC', 12))
        self.passwordentry.grid(row=3, column=1, sticky="ew")
        self.reenterpasswordentry =Entry(self.frame,font=('Bradley Hand ITC', 12))
        self.reenterpasswordentry.grid(row=4, column=1, sticky="ew")
        self.submitregister =Button(self.frame, text="Register", bg="#808080", fg="Black", justify="center", pady=5,padx=20,command=self.submitbutval,font=('Bradley Hand ITC', 12))
        self.submitregister.grid(row=5, column=1, sticky="e")
        self.loginbutton=Button(self.frame,text="Go to Login Page",padx=10,pady=5,bg="green",fg="white",command=self.loginwindowshow,font=('Bradley Hand ITC', 12))
    ...
    def loginwindowshow(self):
        if not self.loginwindow:
            ...
            self.registerwindowshowbtn=Button(self.loginwindow,text="Register",fg="white",bg="green",command=self.registerwindowshow,font=('Bradley Hand ITC', 12))
            self.registerwindowshowbtn.place(x=40,y=200)
            self.loginlabel=Label(self.loginwindow,text="Login To Your Account",font=('Bradley Hand ITC', 20),fg="green").place(x=70,y=65)
            self.loginlabelname=Label(self.loginwindow,text="Enter User Name:",font=('Bradley Hand ITC', 10)).place(x=40,y=120)
            self.loginname=Entry(self.loginwindow,width=35,font=('Bradley Hand ITC', 10))
            #self.loginname.insert(0,'Enter User Id')
            self.loginname.place(x=155,y=120)
            #self.loginname.bind('<Button-1>',self.clearnametext)
            self.loginlabelpassword=Label(self.loginwindow,text="Enter Password:",font=('Bradley Hand ITC', 10)).place(x=40,y=160)
            self.loginpassword=Entry(self.loginwindow,width=35,font=('Bradley Hand ITC', 10))
            ...
            self.loginerrorlabel=Label(self.loginwindow,fg="red",font=('Bradley Hand ITC', 10))
            self.loginerrorlabel.place(x=40,y=250)

            self.submitloginbutton=Button(self.loginwindow,text="Login to your Account",bg="green",fg="white",command=self.loginaccount,font=('Bradley Hand ITC', 12))
        ...
        if self.loginname.get() =="" or self.loginpassword.get() == "" or self.loginname.get() =="Enter User Id" or self.loginpassword.get() == "Enter Password to Login" :
            ...
            if self.db.rowcount == 0:
                ...
                if self.db.rowcount == 0:
                    ...
                    self.forgotbutton = Button(self.loginwindow, text="Forgot Password", bg="red", fg="white",command=self.forgotpaasword,font=('Bradley Hand ITC', 12))
                    ...
                    self.loginwindow.destroy()
                    self.registerwindow.destroy()
                    registrationapply.main(self.loginnameval)
                    # for i in self.db:
                    #    print("values:"+str(i))
                self.conn.commit()
    def forgotpaasword(self):
        ...
        self.gmaillabel = Label(self.forgotFrame, text="Your Gmail:", padx=10, pady=10,font=('Bradley Hand ITC', 10))
        self.gmaillabel.grid(row=0, column=0, sticky="ew")
        self.phonelabel = Label(self.forgotFrame, text="Enter Your phone:", padx=10, pady=10,font=('Bradley Hand ITC', 10))
        self.phonelabel.grid(row=1, column=0, sticky="ew")
        self.getotp = Button(self.forgotFrame, text="Get Otp",command=self.getotp,font=('Bradley Hand ITC', 10))
        self.getotp.grid(row=2, column=1,sticky="e")

        self.gmailentry = Entry(self.forgotFrame,font=('Bradley Hand ITC', 10))
        self.gmailentry.insert(END,self.loginname.get())
        self.gmailentry.config(state="disabled")
        self.gmailentry.grid(row=0, column=1, sticky="ew")
        self.phoneentry = Entry(self.forgotFrame,font=('Bradley Hand ITC', 10))
        self.phoneentry.grid(row=1, column=1, sticky="ew")
        self.errorlabel=Label(self.forgotFrame1,fg="red",anchor="w",font=('Bradley Hand ITC', 10))
    ...
    def getotp(self):
        ...
        if self.phoneentry.get()=="":
            ...
            if lst[0][3]==self.phoneentry.get():
                self.errorlabel.config(text="")
                self.otpentry = Entry(self.forgotFrame,width=15,font=('Bradley Hand ITC', 10))
    ...
    def submitotp(self):
        if self.otpentry.get()=="782946":
            self.errorlabel.config(text="")
            self.passwordentry = Entry(self.forgotFrame, width=15,font=('Bradley Hand ITC', 10))
            ...
            self.repasswordentry = Entry(self.forgotFrame, width=15,font=('Bradley Hand ITC', 10))
            ...
            self.updatepasswordbtn=Button(self.forgotFrame,font=('Bradley Hand ITC', 10),text="Update Password",bg="green",fg="white",command=self.updatepassword)
